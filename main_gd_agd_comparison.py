...
step_gd = dict(zip(range(T_gd + 1), [pd.DataFrame(np.zeros((len(ks), 2)), columns=["x[0]", "x[1]"]) for i in range(T_gd + 1)]))
step_he_gd = dict(zip(range(T_gd + 1), [pd.DataFrame(np.zeros((len(ks), 2)), columns=["x[0]", "x[1]"]) for i in range(T_gd + 1)]))
f_step_gd = pd.DataFrame(np.zeros((len(ks), T_gd + 1)))
f_step_he_gd = pd.DataFrame(np.zeros((len(ks), T_gd + 1)))
step_agd = dict(zip(range(T_agd + 1), [pd.DataFrame(np.zeros((len(ks), 2)), columns=["x[0]", "x[1]"]) for i in range(T_agd + 1)]))
step_he_agd = dict(zip(range(T_agd + 1), [pd.DataFrame(np.zeros((len(ks), 2)), columns=["x[0]", "x[1]"]) for i in range(T_agd + 1)]))
...
for i, k in enumerate(ks):
    ...
    xs_gd_dec = [decrypt_array(val, decryptor, ckks_encoder, 2, 2) for val in xs_gd_enc]
    ...
    xs_agd_enc = he_agd_qp_ckks(Q_enc, p_enc, d, alpha, beta, T_agd, x0_enc, y0_enc, evaluator,
                            ckks_encoder, gal_keys, relin_keys, encryptor, scale)
    xs_agd_dec = [decrypt_array(val, decryptor, ckks_encoder, 2, 2) for val in xs_agd_enc]
...
ax = (f_step_he_gd - f(x_opt)[0][0]).boxplot()
ax.set_yscale('log')
plt.show()
