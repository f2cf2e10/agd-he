import numpy as np
from agd.seal.seal import Encryptor, Evaluator, CKKSEncoder, BatchEncoder, \
    Ciphertext, GaloisKeys, RelinKeys, Plaintext
from agd.matrix.ours import matrix_multiplication as ours_ckks
from agd.matrix.ours import matrix_multiplication_bfv as ours_bfv
from agd.matrix.utils import encrypt_array, squarify, rescale_and_mod_switch_y_and_add_x, encode
...
def he_gd_qp_ckks(Q_enc: Ciphertext, p_enc: Ciphertext, d: int, alpha: float, beta: float, T: int,
    ...
    for t in range(1, T + 1):
        Q_dot_x_enc = ours_ckks(Q_enc, x_enc, evaluator,
...
def he_gd_qp_bfv(Q_enc: Ciphertext, p_enc: Ciphertext, d: int, alpha: float, beta: float, T: int,
                 x0_enc: Ciphertext, evaluator: Evaluator, encoder: BatchEncoder,
                 gal_keys: GaloisKeys, relin_keys: RelinKeys, encryptor: Encryptor,
...
def he_random_gd_qp(Q_enc: Ciphertext, p_enc: Ciphertext, d: int, alpha: float, beta: float, T: int,
    ...
    for t in range(1, T + 1):
        x = ours_ckks(I, x, evaluator, encoder, gal_keys, relin_keys, d, 1., scale)
